...
G_BS = (S = 1/sqrt(2)*QObj([1 (-1) 0 0; 1 1 0 0; 0 0 1 (-1); 0 0 1 1]) ⊗I, L = QObj([0 0 0 0 ])'⊗I, H = QObj([0])⊗ I)
G_pad = (S = QObj([1 0; 0 1] ⊗ I.data), L = QObj([0; 0] ⊗ I.data), H = QObj([0] ⊗ I.data)) # this is just a wire segment. It's the trivial component.
# a beamsplitter interacts with four channels. I number them like this: right upper, right lower, left upper, left lower. notice that there is no scatttering between right and left modes
...
G_BS_pair = feedback_reduce(G_BS_pair, [[1 2 7 8], [5 6 3 4]], dim)
G_BS_odd = concatenate_vec([G_BS_pair for _ in 1:N])
G_BS_even = concatenate_vec([G_pad; [G_BS_pair for _ in 1:(N-1)]; G_pad])
G_cavs_kerr = [(S = I, L = as[j], H =0*as[j]'*as[j] + 0*χ3*as[j]'*as[j]'*as[j]*as[j]) for j in 1:2*N]
G_cavs_kerr = concatenate_vec(G_cavs_kerr)



Gt = concatenate(G_mirs, G_BS_odd)
rights = [i + 2*N for i in 1:4*N if mod(i-1, 4) < 2]
lefts = [i + 2*N for i in 1:4*N if mod(i-1, 4) > 1]
Gt = feedback_reduce(Gt, [[1:2*N; lefts], [rights; 1:2*N]], dim)
# two-way cascade of mirror and BS layer into each other

rights_even = [1; [i + 2*N for i in 3:(4*N-2) if mod(i-1, 4) > 1]; 4*N-1]
lefts_even = [2; [i + 2*N for i in 3:(4*N-2) if mod(i-1, 4) < 2]; 4*N]

Gt = concatenate(Gt, G_BS_even)
Gt = feedback_reduce(Gt, [[1:2*N; lefts_even], [rights_even; 1:2*N]], dim)

Gt = concatenate(Gt, G_BS_odd)
Gt = feedback_reduce(Gt, [[1:2*N; lefts], [rights; 1:2*N]], dim)

Gt = concatenate(Gt, G_cavs_kerr)
rights_1 = [i for i in 1:2*N if mod(i-1, 4) < 2]
lefts_1 = [i for i in 1:2*N if mod(i-1, 4) > 1]

println("Gt cav: ", size(Gt.L.data))

Gt = feedback_reduce(Gt, [[2*N+1:4*N; 1:2*N], [1:2*N; 2*N+1:4*N]], dim)

println("outs: ", [rights_1; 2*N+1:3*N])
println("ins: ", [3*N+1:4*N; lefts_1])
